...
public class CorsConfig {
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        // learn;允许 Vue 开发服务器源和 Nginx 部署源
        configuration.setAllowedOrigins(Arrays.asList("http://localhost:5173", "http://localhost:8888"));
        configuration.setAllowedMethods(Arrays.asList("GET", "POST", "PUT", "DELETE", "OPTIONS")); // learn;允许的方法
        configuration.setAllowedHeaders(Arrays.asList("Authorization", "Cache-Control", "Content-Type", "X-Requested-With")); // 允许的头部
        configuration.setAllowCredentials(true); // learn;是否允许发送 Cookie
        configuration.setMaxAge(3600L); // learn;预检请求的有效期
